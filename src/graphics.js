import { msToString } from './utils.js'

function isActive(graphic) {
	return ['onair', 'coming', 'going'].includes(graphic.status)
}

function isRunning(graphic) {
	return ['running'].includes(graphic.state)
}

export const GraphicTypes = {
	LowerThird: 'lower_third',
	Message: 'message',
	Time: 'time',
	TimeCountdown: 'time_countdown',
	TimeCountup: 'time_countup',
	TimeTod: 'time_tod',
	TimeToTod: 'time_to_tod',
	Image: 'image',
	Ticker: 'ticker',
	Social: 'social',
	Webpage: 'webpage',
	Score: 'score',
	BigTime: 'big_time',
	LowerThirdAnimated: 'lower_third_animated',
	IconWithMessage: 'icon_with_message',
	Credits: 'credits',
	ImageWithMessage: 'image_with_message',
	Video: 'video',
	Celebration: 'celebration',
	AnimatedBackground: 'animated_background',
	NowNextThen: 'now_next_then',
	Checklist: 'checklist',
	QR: 'qr',
	Map: 'map',
}

export const TimerTypes = {
	Countdown: 'countdown',
	Countup: 'countup',
	TimeOfDay: 'time_of_day',
	ToTimeOfDay: 'to_time_of_day',
}

export function findGraphic(type) {
	return Graphics[type] || Graphics['default']
}

const Graphics = {
	[GraphicTypes.LowerThird]: {
		png: '',
		bgColour: [202, 138, 4],
		label(graphic) {
			return `${graphic.line_one}, ${graphic.line_two} (Lower third - ${graphic.id})`
		},
		contents(graphic) {
			return `eek ${graphic.line_one}, ${graphic.line_two}`
		},
	},
	[GraphicTypes.Message]: {
		png: '',
		bgColour: [234, 179, 8],
		label(graphic) {
			return `${graphic.body} (Message) - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.body}`
		},
	},
	[GraphicTypes.Time]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			if (graphic.timerType === TimerTypes.ToTimeOfDay) {
				return `${isActive(graphic) ? msToString(graphic.timeLeft * 1000) : graphic.endTime} (Time - ${graphic.id})`
			}
			if (graphic.timerType === TimerTypes.TimeOfDay) {
				return `Current time of day (Time - ${graphic.id})`
			}

			if ([TimerTypes.Countdown, TimerTypes.Countup].includes(graphic.timerType)) {
				return `${isActive(graphic) ? msToString(graphic.timeLeft) : graphic.duration} (Time - ${graphic.id})`
			}
		},
		contents(graphic) {
			if (graphic.timerType === TimerTypes.ToTimeOfDay) {
				return `${isActive(graphic) ? msToString(graphic.timeLeft * 1000) : graphic.endTime}`
			}
			if (graphic.timerType === TimerTypes.TimeOfDay) {
				return `Current time of day`
			}

			if ([TimerTypes.Countdown, TimerTypes.Countup].includes(graphic.timerType)) {
				return `${isActive(graphic) ? msToString(graphic.timeLeft) : graphic.duration}`
			}

			return `${isActive(graphic) ? msToString(graphic.timeLeft) : graphic.duration}`
		},
	},

	[GraphicTypes.TimeCountdown]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			return `${msToString(graphic.duration)} (Countdown timer - ${graphic.id})`
		},
		contents(graphic) {
			return isRunning(graphic) ? `${graphic.endAt}` : `${msToString(graphic.duration)}`
		},
	},
	[GraphicTypes.TimeCountup]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			return `${msToString(graphic.duration)} (Countup timer - ${graphic.id})`
		},
		contents(graphic) {
			return isRunning(graphic) ? `${graphic.endAt}` : `${msToString(graphic.duration)}`
		},
	},
	[GraphicTypes.TimeTod]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			return `Time of day (Time of Day - ${graphic.id})`
		},
		contents(_graphic) {
			return `Time of day`
		},
	},
	[GraphicTypes.TimeToTod]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			return `To time of day (To time of Day - ${graphic.id})`
		},
		contents(_graphic) {
			return `To time of day`
		},
	},
	[GraphicTypes.Image]: {
		png: '',
		bgColour: [37, 99, 235],
		label(graphic) {
			return `${graphic.name} (Image - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.name}`
		},
	},
	[GraphicTypes.Ticker]: {
		png: '',
		bgColour: [165, 180, 252],
		label(graphic) {
			return `${graphic.title} (Ticker - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.title}`
		},
	},
	[GraphicTypes.Social]: {
		png: '',
		bgColour: [126, 34, 206],
		label(graphic) {
			return `Social - ${graphic.id}`
		},
		contents(graphic) {
			return `${graphic.chat.snippet.displayMessage}`
		},
	},
	[GraphicTypes.Webpage]: {
		png: '',
		bgColour: [55, 48, 163],
		label(graphic) {
			return `${graphic.url} (Webpage - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.url}`
		},
	},
	[GraphicTypes.Score]: {
		png: '',
		bgColour: [34, 197, 94],
		label(graphic) {
			return `Score - ${graphic.id}`
		},
		contents(_graphic) {
			return `Score`
		},
	},
	[GraphicTypes.BigTime]: {
		png: '',
		bgColour: [248, 113, 113],
		label(graphic) {
			return `${isActive(graphic) ? msToString(graphic.timeLeft) : graphic.duration} (Big timer - ${graphic.id})`
		},
		contents(graphic) {
			return `${isActive(graphic) ? msToString(graphic.timeLeft) : graphic.duration}`
		},
	},
	[GraphicTypes.LowerThirdAnimated]: {
		png: '',
		bgColour: [250, 204, 21],
		label(graphic) {
			return `${graphic.line_one}, ${graphic.line_two} (LT Animated - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.line_one}, ${graphic.line_two}`
		},
	},
	[GraphicTypes.IconWithMessage]: {
		png: '',
		bgColour: [234, 179, 8],
		label(graphic) {
			return `${graphic.body} (Message - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.body}`
		},
	},
	[GraphicTypes.Credits]: {
		png: '',
		bgColor: [34, 197, 94],
		label(graphic) {
			return `${graphic.lead} (Credits - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.lead}`
		},
	},
	[GraphicTypes.ImageWithMessage]: {
		png: '',
		bgColour: [150, 103, 28],
		label(graphic) {
			return `${graphic.body} (Image with Message - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.body}`
		},
	},
	[GraphicTypes.Video]: {
		png: '',
		bgColour: [80, 74, 226],
		label(graphic) {
			return `${graphic.name} (Video - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.name}`
		},
	},
	[GraphicTypes.Celebration]: {
		png: '',
		bgColor: [111, 41, 203],
		label(graphic) {
			return `${graphic.celebrationType} (Celebration - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.celebrationType}`
		},
	},
	[GraphicTypes.AnimatedBackground]: {
		png: '',
		bgColor: [67, 102, 232],
		label(graphic) {
			return `${graphic.animationName} (Animated Background - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.animationName}`
		},
	},
	[GraphicTypes.NowNextThen]: {
		png: '',
		bgColor: [50, 87, 68],
		label(graphic) {
			return `${graphic.items[0].sectionTitle} (Now next then - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.items[0].sectionTitle}`
		},
	},
	[GraphicTypes.Checklist]: {
		png: '',
		bgColor: [100, 102, 237],
		label(graphic) {
			return `${graphic.title} (Checklist - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.title}`
		},
	},
	[GraphicTypes.QR]: {
		png: '',
		bgColor: [53, 88, 52],
		label(graphic) {
			return `${graphic.message} (QR code - ${graphic.id})`
		},
		contents(graphic) {
			return `${graphic.message}`
		},
	},
	[GraphicTypes.Map]: {
		png: '',
		bgColor: [45, 60, 135],
		label(graphic) {
			return `Map - ${graphic.id}`
		},
		contents(_graphic) {
			return `Map`
		},
	},
	default: {
		png: 'iVBORw0KGgoAAAANSUhEUgAAAEgAAAA6CAYAAAATBx+NAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAmSURBVHgB7cABAQAAAIIg/69uSFAAAAAAAAAAAAAAAAAAAAAArwZBegAB2I2aJAAAAABJRU5ErkJggg==',
		bgColor: [0, 0, 0],
		label(graphic) {
			return `Default - ${graphic.id}`
		},
		contents(_graphic) {
			return `Default`
		},
	},
}
