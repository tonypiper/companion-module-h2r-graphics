...
export const graphics = {
    ...
    time: {
        ...
        label(graphic) {
            if (graphic.timerType === 'to_time_of_day') {
                return `${isActive(graphic)
                    ? msToString(graphic.timeLeft * 1000)
                    : graphic.endTime
                    } (Time - ${graphic.id})`
            }
            if (graphic.timerType === 'time_of_day') {
                return `Current time of day (Time - ${graphic.id})`
            }

            if (['countdown', 'countup'].includes(graphic.timerType)) {
                return `${isActive(graphic)
    ...
    image_with_message: {
        ...
        bgColour: [150, 103, 28],
        label(graphic) {
            return `${graphic.body} (Image with Mesage - ${graphic.id})`
        },
        contents(graphic) {
            return `${graphic.body}`
        }
